...
const GoogleSimulation: React.FC<GoogleSimulationProps> = ({ searchType = 'tremayne' }) => {
  ...
  useEngagementTracking('tremayne', currentPage, activeTab, footprintCondition, prolificParams);
  ...
  const allResults = useMemo(() => {
    console.log('Loading RESULTS_Tremayne_Washington:', RESULTS_Tremayne_Washington);
    if (!RESULTS_Tremayne_Washington || RESULTS_Tremayne_Washington.length === 0) {
      console.error('RESULTS_Tremayne_Washington is empty or undefined!');
      return [];
    }
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#fff' }}>
        ...
        <div style={{ display: 'flex', gap: isMobile ? '0' : '32px' }}>
          ...
          <div style={{ flex: '1', minWidth: 0, width: '100%' }}>
            ...
            <div style={{ color: '#70757a', fontSize: '14px', marginBottom: '16px' }}>
              About 1,150,000 results (0.45 seconds)
            </div>
      ...
      {selectedResult && selectedResult.platform === 'Instagram' && (
        <InstagramProfile
          ...
          onClose={() => {
            if (selectedResult) {
              trackProfileClose(selectedResult.id, 'Instagram', 'tremayne', footprintCondition, prolificParams);
            }
            setSelectedResult(null);
          }}
        />
      ...
      {selectedResult && selectedResult.platform === 'X' && (
        <XProfile
          ...
          onClose={() => {
            if (selectedResult) {
              trackProfileClose(selectedResult.id, 'X', 'tremayne', footprintCondition, prolificParams);
            }
            setSelectedResult(null);
          }}
        />
