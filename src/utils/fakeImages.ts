...
export const getFakeImageUrl = (resultId: string, type: 'avatar' | 'thumbnail' | 'header' = 'avatar'): string => {
  ...
  // Use profile picture from Photos folder for Tremayne Washington (African American Male)
  if ((resultId.toLowerCase().includes('tremayne') || resultId.toLowerCase().includes('tremayne1')) && (type === 'avatar' || type === 'thumbnail')) {
    return getPersonaProfilePicture('African American', 'Male');
  }
